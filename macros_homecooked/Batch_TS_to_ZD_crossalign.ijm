 ...
 * Work flow:
 * 0---Load the ZD stack 
 * 1---Load the TS stack, make a substack using the first N slices 
 * 2---Register the TS stack to the ZD stack and keep the registration files
 */
...
function TS2ZD_crossalign(work_folder){
	...
	if (n_TS_folder ==1){
		...
		for(j=0;j< ZP_list.length;j++){
			if (endsWith(ZP_list[j], ".tif")){
				...
				zrange = parseInt(zstep_number/2); // The uncertainty within z slice. 
...
macro "Batch_TZcross"{
	run("Close All");
	global_folder = getDirectory("Choose a Directory ");
	flist = getFileList(global_folder);
	for(i=0;i<flist.length;i++){
		print(flist[i]);
		if (endsWith(flist[i], '/')) { // if it is a directory
			TS2ZD_crossalign(global_folder + flist[i]);
		}
	}//end for
