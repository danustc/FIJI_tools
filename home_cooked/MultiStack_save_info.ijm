...
function slice_2_stack(refstack_ID, slice_ID){
	...
	run("MultiStackReg", "stack_1="+ title_ref+" action_1=[Use as Reference] file_1=[] stack_2="+ title_align + " action_2=[Align to First Stack] file_2=[] transformation=[Rigid Body] save");	
...
slice_2_stack(ref_ID,align_ID);
selectImage(align_ID);
run("Save");
close();
selectImage(ref_ID);
run("Save");
